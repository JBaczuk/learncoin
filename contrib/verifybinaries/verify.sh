...
###   learncoincore.org and learncoin.org and compares them.
...
WORKINGDIR="/tmp/learncoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://learncoincore.org"
HOST2="https://learncoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="learncoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'learncoin-',
...
if [ $? -ne 0 ]; then
   echo "learncoin.org failed to provide signature file, but learncoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "learncoin.org and learncoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Learncoin Core binary release signing key installed?"
   fi
